...
class BecomeSellerCustome extends \App\PageBuilder\PageBuilderBase
{
    ...
    public function admin_render()
    {
        ...
        $widget_saved_values = $this->get_settings();
         ...
         $output .= Text::get([
            'name' => 'maintitle',
            'label' => __('Main Title'),
            'value' => $widget_saved_values['maintitle'] ?? null,
        ]);
          $output .= Textarea::get([
            'name' => 'mainsubtitle',
            'label' => __('Main Subtitle'),
            'value' => $widget_saved_values['mainsubtitle'] ?? null,
        ]);
        $output .= Image::get([
            'name' => 'shape_image',
...
return <<<HTML
    ...
    <section class="seller-area" data-padding-top="{$padding_top}" data-padding-bottom="{$padding_bottom}" style="background-color:{$section_bg}">
        <div class="container">
            <div class="row flex-column-reverse flex-lg-row align-items-center justify-content-between">
                <div class="col-xl-12 col-lg-12 margin-top-30 mb-5">
                    <div class="marketplace-left-contents text-center">
                        <h2 class="title" style="font-size:30px!important;"> {$maintitle} </h2>
                        <p class="market-para">{$mainsubtitle}</p>
                        <!--<div class="btn-wrapper">
                            <a href="{$btn_link}" class="cmn-btn btn-bg-3"> {$btn_text} </a>
                        </div>-->
                    </div>
                </div>
                <div class="col-lg-6 margin-top-30 order-2">
                    <div class="seller-wrapper about-thumb-content">
                        <div class="about-shape">
                            {$about_shape}
                        </div>
                        <div class="section-title text-left">
                            <h2 class="title" style="font-size:30px!important;line-height: 40px!important;"> {$title_start} <span style="color:{$title_text_color}"> {$title_end} </span> </h2>
                            <span class="section-para">{$subtitle}</span>
                        </div>
                        <div class="seller-contents">
                            {$content_list_markup}
                            <div class="btn-wrapper" style="text-align:end;">
...
HTML;
    ...
    {
        return __('Start As Seller Custome');
    }
}